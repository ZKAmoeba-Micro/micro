...
micro is currently can be launched on any \*nix operating system (e.g. any linux distribution or MacOS).
...
(e.g. your terminal and IDE don't run in Rosetta, and your toolchain is native). Trying to work with micro code via
...
**Note:** On linux you may encounter the following error when you’ll try to work with `micro`:
...
1.67.1-aarch64-apple-darwin (overridden by '/Users/user/workspace/micro-2-dev/rust-toolchain')
```
...
Go to the micro folder and run `nix develop --impure`. After it finishes, you are in a shell that has all the
dependencies.
...
```bash
# Add path here:
export MICRO_HOME=/path/to/micro

export PATH=$MICRO_HOME/bin:$PATH
