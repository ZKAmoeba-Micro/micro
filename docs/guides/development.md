...
- Generate `$MICRO_HOME/etc/env/dev.env` file with settings for the applications.
...
2. If after getting new functionality from the `main` branch your code stopped working and `zk init` doesn't help, you
   may try removing `$MICRO_HOME/etc/env/dev.env` and running `zk init` once again. This may help if the application
...
`micro` uses pre-commit and pre-push git hooks for basic code integrity checks. Hooks are set up automatically within
