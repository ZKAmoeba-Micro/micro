...
In the case of micro, ecAdd and ecMul precompiles are written as a smart contract for two reasons:
...
Instructions set on micro and EVM are different, so the performance of the same Yul/Solidity code can be efficient on
