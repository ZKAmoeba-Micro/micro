...
micro as well as other L2s have the issue which does not allow to adopt the same model as the one for Ethereum so
...
micro, being a zkRollup is required to prove every operation with zero knowledge proofs. That comes with a few nuances.
...
That’s why you will find the prices for operations on micro a lot different from the ones on Ethereum.
...
balances of the users, the nonce and signature verification, on micro these prices are _not_ included in the intrinsic
...
That being said, micro transactions do come with some small intrinsic costs, but they are mostly used to cover costs
...
In order to process the batch, the micro team has to pay for proving of the batch, committing to it, etc. Processing a
...
case of micro batches, there are some limited resources the batch should manage:
...
- **Pubdata bytes.** In order to fully appreciate the gains from the storage diffs, i.e. the fact that changes in a
  ...
  and so this is the limit that each micro batch should adhere to.
...
[trusted gas limit](https://github.com/code-423n4/2023-10-micro/blob/ef99273a8fdb19f5912ca38ba46d6bd02071363d/code/system-contracts/bootloader/bootloader.yul#L1137),
...
The micro fee model is meant to be the basis of the long-term fee model, which provides both robustness and security.
...
The other distinctive feature of the fee model used on micro is the abundance of refunds, i.e.:

- For unused limited system resources.
- For overpaid computation.

This is needed because of the relatively big upfront payments required in micro to provide DDoS security.
...
`GUARANTEED_PUBDATA_PER_TX` (_PG_) — The guaranteed number of pubdata that should be possible to pay for in one micro
...
- On-Chain L1 Costs
  ...
  - L1 Finalize Withdrawals
    - While not strictly part of the L1 fees, the cost to finalize L2 → L1 withdrawals are covered by Zkamoeba. The
