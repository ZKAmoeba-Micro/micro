...
[page](https://github.com/code-423n4/2023-10-micro/blob/main/docs/Smart%20contract%20Section/Batches%20&%20L2%20blocks%20on%20micro.md)
about the block processing on micro.
...
Note that, unlike Ethereum, keccak256 is a precompile (_not an opcode_) on micro.
...
[here](https://github.com/code-423n4/2023-10-micro/blob/main/docs/Smart%20contract%20Section/Elliptic%20curve%20precompiles.md).
...
This contract is used to store whether a certain code hash is “known”, i.e. can be used to deploy contracts. On micro,
...
`ContractDeployer` is a system contract responsible for deploying contracts on micro. It is better to understand how it
works in the context of how the contract deployment works on micro. Unlike Ethereum, where `create`/`create2` are
opcodes, on micro these are implemented by the compiler via calls to the ContractDeployer system contract.
...
- Have different derivation rules from Ethereum. That is the path that micro has chosen, mainly because since we have
...
On Ethereum, the same nonce is used for CREATE for accounts and EOA wallets. On micro this is not the case, we use a
...
returns the deployedCode. On micro, contracts only have the deployed code and can set immutables as storage variables
...
returns the deployment code of the contract. On micro, there is no separation between deployed code and constructor
...
`ImmutableSimulator.getImmutable(getCodeAddress(), index)`. Note that on micro it is possible to get the current
execution address (you can read more about `getCodeAddress()` [here](#micro-specific-opcodes).
...
- Contains minimal implementation of our account abstraction protocol. Note that it supports the
  [built-in paymaster flows](https://v2-docs.micro.io/dev/developer-guides/aa.html#paymasters).
...
A contract used for sending arbitrary length L2→L1 messages from micro to L1. While micro natively supports a rather
...
- Fee modeling is yet to be improved. More on it in the
  [document](https://github.com/code-423n4/2023-10-micro/blob/main/docs/Smart%20contract%20Section/micro%20fee%20model.md)
  on the fee model.
