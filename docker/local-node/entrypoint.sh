...
then
    ...
    sed -i 's!^path=.*$!path="/var/lib/micro/data"!' /etc/env/base/database.toml
    sed -i 's!^state_keeper_db_path=.*$!state_keeper_db_path="/var/lib/micro/data/state_keeper"!' /etc/env/base/database.toml
    sed -i 's!^merkle_tree_backup_path=.*$!merkle_tree_backup_path="/var/lib/micro/data/backups"!' /etc/env/base/database.toml

    # Switch zksolc compiler source from docker to binary
    sed -i "s!'docker'!'binary'!" /contracts/micro/hardhat.config.ts
