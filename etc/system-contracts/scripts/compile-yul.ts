...
function preparePaths(path: string, files: string[], outputDirName: string | null): CompilerPaths {
    ...
    let absolutePathSources = `${process.env.MICRO_HOME}/etc/system-contracts/${path}`;

    let absolutePathArtifacts = `${process.env.MICRO_HOME}/etc/system-contracts/${path}/artifacts`;
