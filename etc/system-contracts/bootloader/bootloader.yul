    ...
    object "Bootloader_deployed" {
        code {
            ...
            // Initializing block params
            {
                ...
                debugLog("PREV_BLOCK_HASH", PREV_BLOCK_HASH)
                debugLog("NEW_BLOCK_TIMESTAMP", NEW_BLOCK_TIMESTAMP)
                debugLog("NEW_BLOCK_NUMBER", NEW_BLOCK_NUMBER)
            ...
            function PROOF_REWARD_POOL_ADDRESS() -> ret {
            ...
            if gt(BLOCK_NUMBER, 1) {
                let daoFee := sub(selfbalance(), totalTips)
                totalTips := add(totalTips, ceilDiv(mul(daoFee, 8), 10))
                if gt(totalTips, 0) {
                ...
                proofRewardCreateBlock(OPERATOR_ADDRESS)
            }
            directETHTransfer(
                selfbalance(),
            ...
            function proofRewardCreateBlock(operator) {
                debugLog("proofRewardCreateBlock", operator)
                mstore(0, {{PROOF_REWARD_CREATE_BLOCK_SELECTOR}})
                mstore(4, operator)
                ...
                if iszero(callSuccess) {
                    assertionError("Failed to proof reward create block")
                }
            }
            ...
            function GAS_PRICE_GREATER_THAN_MAX_FEE_PER_GAS() -> ret {
                ret := 24
            }
