...
fn get_vk_json(vk: &VerificationKey<Bn256, MicroCircuit<Bn256, VmWitnessOracle<Bn256>>>) -> Value {
    serde_json::to_value(vk).unwrap()
}

fn get_vk_jsons(
    vks: Vec<VerificationKey<Bn256, MicroCircuit<Bn256, VmWitnessOracle<Bn256>>>>,
) -> Vec<Value> {
    vks.into_iter().map(|vk| get_vk_json(&vk)).collect()
}

fn compare_vks(
    first: Vec<VerificationKey<Bn256, MicroCircuit<Bn256, VmWitnessOracle<Bn256>>>>,
    second: Vec<VerificationKey<Bn256, MicroCircuit<Bn256, VmWitnessOracle<Bn256>>>>,
    error_message: &str,
) {
