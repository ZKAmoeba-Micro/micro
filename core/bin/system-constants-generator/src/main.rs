...
use micro_types::{
    IntrinsicSystemGasConstants, DEPLOY_L2_CONTRACT_TX_GAS_LIMIT, GUARANTEED_PUBDATA_IN_TX,
...
fn save_file(path_in_repo: &str, content: String) {
    let micro_home = std::env::var("MICRO_HOME").expect("No MICRO_HOME env var");
    let fee_constants_path = format!("{micro_home}/{path_in_repo}");
