...
use micro_basic_types::L2ChainId;
...
use micro_core::{
    ...
    state_keeper::{
        batch_executor::MainBatchExecutorBuilder, seal_criteria::SealManager, MicroStateKeeper,
    },
    sync_layer::{
...
async fn init_tasks(
    ...
    let private_key = config.eth_sender.sender.operator_private_key.clone();
    let check_heartbeat = task::spawn_blocking(move || {
        check_heartbeat::run(l2_node_heart_beat_url.as_str(), private_key);
    });
...
async fn main() -> anyhow::Result<()> {
    ...
    .await;
    vlog::info!("Started the external node init_tasks.");
    let (task_handles, stop_sender) = init_tasks(config.clone(), connection_pool.clone()).await;
    vlog::info!("Started the external node init_tasks end.");
