...
use micro_config::configs::house_keeper::HouseKeeperConfig;
use micro_config::configs::{ProverGroupConfig, WitnessGeneratorConfig};
use micro_config::{MicroConfig, ProverConfigs};
use micro_dal::healthcheck::ConnectionPoolHealthCheck;
use micro_dal::{ConnectionPool, StorageProcessor};
use micro_eth_client::clients::http::PKSigningClient;
use micro_eth_client::BoundEthInterface;
use micro_health_check::CheckHealth;
use micro_mempool::MempoolStore;
...
use crate::node_check::CheckNode;
...
/// Inserts the initial information about micro tokens into the database.
pub async fn genesis_init(config: MicroConfig) {
    let mut storage = StorageProcessor::establish_connection_blocking(true);
    genesis::ensure_genesis_state(
        &mut storage,
        L2ChainId(config.chain.eth.micro_network_id),
...
impl FromStr for Components {
    ...
    fn from_str(s: &str) -> Result<Components, String> {
        match s {
            ...
            "node_check" => Ok(Components(vec![Component::NodeCheck])),
...
pub async fn initialize_components(
    ...
    if components.contains(&Component::NodeCheck) {
        let l2_node_heart_beat_url = config.chain.state_keeper.l2_node_heart_beat_url.clone();
        vlog::info!("initializing NodeCheck url:{}", l2_node_heart_beat_url);
        // let node_heartbeat = [tokio::spawn(node_check::run(
        //     l2_node_heart_beat_url.clone(),
        //     stop_receiver.clone(),
        // ))];
        // task_futures.extend(node_heartbeat);

        let check_node = CheckNode::new(l2_node_heart_beat_url.clone());
        let check_dele = check_node.clone();
        task_futures.extend([
            tokio::spawn(check_node.run(stop_receiver.clone())),
            tokio::spawn(check_dele.delete_heartbeat_address(stop_receiver.clone())),
        ]);
    }
...
fn add_witness_generator_to_task_futures(
    ...
    if std::env::var("MICRO_LOCAL_SETUP") == Ok("true".to_owned()) {
        return;
    }
