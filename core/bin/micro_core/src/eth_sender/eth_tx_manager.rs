...
use micro_types::{
    ...
    L1BlockNumber, DEPLOY_L2_CONTRACT_TX_GAS_LIMIT, H256, U256,
};
use micro_utils::time::seconds_since_epoch;

use crate::{eth_sender::ETHSenderError, l1_gas_price::L1GasPriceProvider};
...
impl<E, G, F> EthTxManager<E, G, F>
where
    E: BoundEthInterface + Sync,
    G: L1TxParamsProvider,
    F: L1GasPriceProvider,
{
    pub fn new(
        config: SenderConfig,
        gas_adjuster: Arc<G>,
