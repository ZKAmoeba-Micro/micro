...
pub fn agg_block_base_cost(op: AggregatedActionType) -> u64 {
    match op {
...
pub fn block_base_cost(op: AggregatedActionType) -> u64 {
    match op {
...
pub trait GasCost {
    fn base_cost(&self, op: AggregatedActionType) -> u64;
}
...
pub fn commit_gas_count_for_block(block: &BlockWithMetadata) -> u64 {
    let base_cost = block_base_cost(AggregatedActionType::CommitBlocks);
    let additional_calldata_bytes = block.metadata.initial_writes_compressed.len() as u64
        + block.metadata.repeated_writes_compressed.len() as u64
        + block.metadata.l2_l1_messages_compressed.len() as u64
        + block
            .header
            .l2_to_l1_messages
            .iter()
