...
use micro_eth_client::{clients::http::PKSigningClient, EthInterface};
use micro_types::{
...
impl FeeMonitor {
    pub fn new(config: &MicroConfig, storage: ConnectionPool, client: PKSigningClient) -> Self {
    ...
    async fn report_balances(&self) -> anyhow::Result<()> {
        ...
        let mut fee_account_balance_l2 = storage
            ...
            .standard_token_historical_balance(
                ...
                BlockId::Number(micro_types::api::BlockNumber::Pending),
            )??
            .to_f64_with_decimals(18);
