...
use micro_merkle_tree::MicroTree as OldTree;
use micro_merkle_tree2::domain::MicroTree as NewTree;
use micro_object_store::ObjectStore;
use micro_storage::RocksDB;
use micro_types::{block::WitnessBlockWithLogs, L1BatchNumber};

use super::{
    get_logs_for_l1_batch,
    helpers::{Delayer, MicroTree},
...
impl TreeUpdater {
    pub fn new(
        ...
        let tree = match mode {
            MetadataCalculatorMode::Full(Old) => MicroTree::Old(OldTree::new(db)),
            MetadataCalculatorMode::Full(New) => MicroTree::New(NewTree::new(db)),
            MetadataCalculatorMode::Lightweight(Old) => {
                MicroTree::Old(OldTree::new_lightweight(db))
            }
            MetadataCalculatorMode::Lightweight(New) => {
                MicroTree::New(NewTree::new_lightweight(db))
            }
        };
    ...
    fn tree_implementation(&self) -> TreeImplementation {
        match &self.tree {
            MicroTree::Old(_) => TreeImplementation::Old,
            MicroTree::New(_) => TreeImplementation::New,
        }
    }
