...
#[derive(Debug, Parser)]
#[structopt(author = "Matter Labs", version, about = "micro operator node", long_about = None)]
struct Cli {
    ...
    #[arg(
        long,
        default_value = "api,tree,tree_lightweight,eth,data_fetcher,state_keeper,witness_generator,housekeeper,node_check"
    )]
...
async fn main() -> anyhow::Result<()> {
    ...
    if sentry_guard.is_some() {
        vlog::info!(
            ...
            env::var("CHAIN_ETH_MICRO_NETWORK").unwrap(),
        );
    } else {
