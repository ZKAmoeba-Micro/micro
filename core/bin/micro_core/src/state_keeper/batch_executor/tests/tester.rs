...
use micro_contracts::{get_loadnext_contract, TestContract};
...
use micro_utils::{test_utils::LoadnextContractExecutionParams, u256_to_h256};
use tempfile::TempDir;
use vm::{
    test_utils::{
        get_create_micro_address, get_deploy_tx, mock_loadnext_gas_burn_call,
...
const CHAIN_ID: L2ChainId = L2ChainId(380);
...
impl Account {
    ...
    pub(super) fn deploy_loadnext_tx(&mut self) -> (Transaction, Address) {
        ...
        let test_contract_address =
            get_create_micro_address(loadnext_deploy_tx.initiator_account(), self.nonce);
        self.nonce += 1;
