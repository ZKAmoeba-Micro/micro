...
use micro_types::{Address, L1BatchNumber, MiniblockNumber, Transaction, U256};
...
use crate::state_keeper::{mempool_actor::l2_tx_filter, SystemContractCaller};
use crate::{
    ...
    fn wait_for_new_batch_params(&mut self, max_wait: Duration) -> Option<L1BatchParams> {
        ...
        poll_until(self.delay_interval, max_wait, || {
            ...
            self.mempool.has_next(&self.filter).then(|| {
                ...
                let node_list = self.system_contract_caller.deposit_node_list().unwrap();
                if !node_list.is_empty() {
                    let block_number = U256::from(self.current_l1_batch_number.0);
                    let len = node_list.len();
                    let len: U256 = U256::from(len);
                    let (_, mod_result) = block_number.add(previous_l1_batch_hash).div_mod(len);
