...
use super::{
    keeper::POLL_WAIT_DURATION, seal_criteria::conditional_sealer::ConditionalSealer,
...
pub fn default_system_contract_caller(connection_pool: ConnectionPool) -> SystemContractCaller {
    SystemContractCaller::new(
        connection_pool,
        250000000,
...
#[db_test]
async fn test_system_contract_caller(connection_pool: ConnectionPool) {
    let caller = default_system_contract_caller(connection_pool);
    let res = caller.deposit_node_list();
    println!("{:?}", res);
}
