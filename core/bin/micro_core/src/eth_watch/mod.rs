...
//! New events are accepted to the micro network once they have the sufficient amount of L1 confirmations.
//!
...
impl<W: EthClient> EthWatch<W> {
    ...
    async fn loop_iteration(&mut self, storage: &mut StorageProcessor<'_>) -> Result<(), Error> {
        ...
        let mut to_block = Self::get_current_finalized_eth_block(
            &self.client,
            self.number_of_confirmations_for_event,
        )
        .await?;
        ...
        if to_block > self.state.last_processed_ethereum_block + 2880 {
            to_block = self.state.last_processed_ethereum_block + 2880;
        }
    ...
    async fn get_new_priority_ops(
        ...
        if !priority_ops.is_empty() {
            for p in &priority_ops {
                vlog::debug!(
                    "priority serial id: {}, block: {}, tx_hash: {}",
                    p.serial_id(),
                    p.eth_block(),
                    p.hash()
                );
