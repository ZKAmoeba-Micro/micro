...
#[derive(Debug, Parser)]
#[command(author = "Zkamoeba", version, about = "micro operator node", long_about = None)]
struct Cli {
    ...
    #[arg(
        long,
        default_value = "api,tree,eth,data_fetcher,state_keeper,housekeeper,basic_witness_input_producer,l2_sender,assignments"
    )]
...
async fn main() -> anyhow::Result<()> {
    ...
    // Right now, we are trying to deserialize all the configs that may be needed by `micro_core`.
