...
pub struct App<'a> {
    ...
    pub db: micro_dal::StorageProcessor<'a>,
}

pub fn create_app<'a>(profile: &Option<String>) -> Result<App<'a>, AppError> {
    if profile.is_some() {
        let home = std::env::var("MICRO_HOME").map_err(|x| AppError::Init(InitError::Env(x)))?;
    ...
    let db = micro_dal::StorageProcessor::establish_connection_blocking(true);
