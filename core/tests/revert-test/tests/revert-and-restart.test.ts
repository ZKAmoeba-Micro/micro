...
async function killServerAndWaitForShutdown(tester: Tester) {
    await utils.exec('pkill -9 micro_server');
...
describe('Block reverting test', function () {
    ...
    step('revert blocks', async () => {
        ...
        await utils.spawn(
            `cd $MICRO_HOME && cargo run --bin block_reverter --release -- send-eth-transaction --l1-batch-number ${lastL1BatchNumber} --nonce ${nonce} --priority-fee-per-gas ${priorityFee}`
        );

        console.log('Rolling back DB..');
        await utils.spawn(
            `cd $MICRO_HOME && cargo run --bin block_reverter --release -- rollback-db --l1-batch-number ${lastL1BatchNumber} --rollback-postgres --rollback-tree --rollback-sk-cache`
        );
    ...
    after('Try killing server', async () => {
        await utils.exec('pkill micro_server').catch(ignoreError);
    });
});
