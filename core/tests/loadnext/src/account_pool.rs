...
/// An alias to [`micro::Wallet`] with HTTP client. Wrapped in `Arc` since
...
impl AccountPool {
    ...
    pub async fn new(config: &LoadtestConfig) -> anyhow::Result<Self> {
        ...
        let client = HttpClientBuilder::default()
            ...
            .unwrap();
        // Perform a health check: check whether micro server is alive.
        ...
        if !server_alive {
            anyhow::bail!("micro server does not respond. Please check RPC address and whether server is launched");
        }
