...
use micro_types::REQUIRED_L1_TO_L2_GAS_PER_PUBDATA_BYTE;
use std::ops::Add;
use tokio::task::JoinHandle;

use micro::ethereum::{PriorityOpHolder, DEFAULT_PRIORITY_FEE};
use micro::utils::{
    get_approval_based_paymaster_input, get_approval_based_paymaster_input_for_estimation,
};
use micro::web3::{contract::Options, types::TransactionReceipt};
use micro::{EthereumProvider, ZksNamespaceClient};
...
use micro_types::api::{BlockNumber, U64};
use micro_types::{tokens::ETHEREUM_ADDRESS, Address, Nonce, U256};
...
impl Executor {
    ...
    /// but also to not be close to the supported limits in micro.
...
async fn deposit_with_attempts(
    ...
    for attempt in 1..=max_attempts {
        ...
        // Wait for the corresponding priority operation to be committed in micro.
