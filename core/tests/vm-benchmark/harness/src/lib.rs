...
use micro_contracts::deployer_contract;
use micro_state::storage_view::StorageView;
use micro_types::ethabi::{encode, Token};
use micro_types::l2::L2Tx;
use micro_types::utils::storage_key_for_eth_balance;
use micro_types::zk_evm::block_properties::BlockProperties;
use micro_types::{fee::Fee, Nonce, Transaction, H256, U256};
use micro_types::{L2ChainId, PackedEthSignature, CONTRACT_DEPLOYER_ADDRESS};
...
static STORAGE: Lazy<InMemoryStorage> = Lazy::new(|| {
    ...
    storage.set_value(key, micro_utils::u256_to_h256(U256([0, 0, 1, 0])));
...
#[cfg(test)]
mod tests {
    use micro_contracts::read_bytecode;
    use micro_types::tx::tx_execution_info::TxExecutionStatus::Success;
