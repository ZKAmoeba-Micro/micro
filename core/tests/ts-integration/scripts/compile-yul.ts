...
import { getZksolcPath, getZksolcUrl, saltFromUrl } from '@zkamoeba/hardhat-micro-solc';
...
async function compilerLocation(): Promise<string> {
    if (IS_COMPILER_PRE_RELEASE) {
        const url = getZksolcUrl('https://github.com/zkamoeba/zksolc-prerelease', hre.config.zksolc.version);
...
function preparePaths(path: string, files: string[], outputDirName: string | null): CompilerPaths {
    ...
    let absolutePathSources = `${process.env.MICRO_HOME}/core/tests/ts-integration/${path}`;

    let absolutePathArtifacts = `${process.env.MICRO_HOME}/core/tests/ts-integration/${path}/artifacts`;
