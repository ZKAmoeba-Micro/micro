...
export async function toBeReverted(
    txPromise: micro.types.TransactionResponse | Promise<micro.types.TransactionResponse>,
    modifiers: MatcherModifier[] = [],
    additionalInfo?: string
) {
    try {
 ...
 */
function checkReceiptFields(request: micro.types.TransactionRequest, receipt: micro.types.TransactionReceipt) {
