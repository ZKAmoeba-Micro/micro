...
export class TestContextOwner {
    ...
    private mainSyncWallet: micro.Wallet;

    private l1Provider: ethers.providers.JsonRpcProvider;
    private l2Provider: micro.Provider;
    ...
    constructor(env: TestEnvironment) {
        ...
        this.mainSyncWallet = new micro.Wallet(env.mainWalletPK, this.l2Provider, this.l1Provider);
    }
    ...
    private async collectFunds() {
        ...
        const l2Wallets = Object.values(this.wallets!).map(
            (pk) => new micro.Wallet(pk, this.l2Provider, this.l1Provider)
        );
...
export async function sendTransfers(
    token: string,
    wallet: ethers.Wallet | micro.Wallet,
    wallets: TestWallets,
    ...
    const erc20Contract =
        wallet instanceof micro.Wallet
            ? new micro.Contract(token, micro.utils.IERC20, wallet)
            : new ethers.Contract(token, micro.utils.IERC20, wallet);
    ...
    const txPromises = Array.from(Object.values(wallets)).map((testWalletPK, index) => {
        if (token == micro.utils.ETH_ADDRESS) {
            const tx = {
...
export async function claimEtherBack(
    wallets: ethers.Wallet[] | micro.Wallet[],
    toAddress: string,
    reporter?: Reporter
): Promise<Promise<any>[]> {
    const promises = [];
