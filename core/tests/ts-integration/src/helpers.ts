 ...
 */
export function getTestContract(name: string): MicroArtifact {
    const artifact = hre.artifacts.readArtifactSync(name);
    return artifact as MicroArtifact;
}
...
export function getContractSource(relativePath: string): string {
    const contractPath = `${process.env.MICRO_HOME}/core/tests/ts-integration/contracts/${relativePath}`;
...
export async function deployContract(
    initiator: micro.Wallet,
    artifact: MicroArtifact,
    args: any[],
    deploymentType?: micro.types.DeploymentType,
    overrides: any = {}
): Promise<micro.Contract> {
    const contractFactory = new micro.ContractFactory(artifact.abi, artifact.bytecode, initiator, deploymentType);
 ...
 */
export async function anyTransaction(wallet: micro.Wallet): Promise<ethers.providers.TransactionReceipt> {
    return await wallet.transfer({ to: wallet.address, amount: 0 }).then((tx) => tx.wait());
}

/**
 * Waits until a new L1 batch is created on micro node.
 ...
 */
export async function waitForNewL1Batch(wallet: micro.Wallet): Promise<micro.types.TransactionReceipt> {
    ...
    while (!(await wallet.provider.getTransactionReceipt(oldReceipt.transactionHash)).l1BatchNumber) {
        await micro.utils.sleep(wallet.provider.pollingInterval);
    }
 ...
 */
export async function waitUntilBlockFinalized(wallet: micro.Wallet, blockNumber: number) {
    while (true) {
 ...
 */
export async function scaledGasPrice(wallet: ethers.Wallet | micro.Wallet): Promise<ethers.BigNumber> {
