...
import { IERC20Factory } from '@zkamoeba/micro-web3/build/typechain';
 ...
 */
export async function shouldOnlyTakeFee(wallet: micro.Wallet, isL1ToL2?: boolean): Promise<ShouldChangeBalance> {
    return await ShouldChangeBalance.create(micro.utils.ETH_ADDRESS, [{ wallet, change: 0 }], { l1ToL2: isL1ToL2 });
}
...
export async function shouldChangeETHBalances(
    ...
    return await ShouldChangeBalance.create(micro.utils.ETH_ADDRESS, balanceChanges, params);
}
...
class ShouldChangeBalance extends MatcherModifier {
    ...
    static async create(token: string, balanceChanges: BalanceChange[], params?: Params) {
        ...
        if (token == micro.utils.ETH_ADDRESS && l1 && !noAutoFeeCheck) {
    ...
    async check(receipt: micro.types.TransactionReceipt): Promise<MatcherMessage | null> {
        ...
        for (const balanceChange of this.balanceChanges) {
            ...
            const autoFeeCheck = !this.noAutoFeeCheck && this.token == micro.utils.ETH_ADDRESS;
            if (autoFeeCheck) {
 ...
 */
export function extractFee(receipt: micro.types.TransactionReceipt, from?: string): Fee {
 ...
 * @returns Extracted fee
 */
function extractRefundForL1ToL2(receipt: micro.types.TransactionReceipt, refundRecipient?: string): ethers.BigNumber {
