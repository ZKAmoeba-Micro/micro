...
declare global {
    var __MICRO_TEST_CONTEXT_OWNER__: TestContextOwner;
}
...
async function performSetup(_globalConfig: any, _projectConfig: any) {
    ...
    process.env.MICRO_JEST_TEST_CONTEXT = JSON.stringify(testContext);

    // Store the context object for teardown script, so it can perform, well, the teardown.
    globalThis.__MICRO_TEST_CONTEXT_OWNER__ = testContextOwner;
}
