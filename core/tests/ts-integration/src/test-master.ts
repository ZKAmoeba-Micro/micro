...
export class TestMaster {
    ...
    private mainWallet: micro.Wallet;
    private subAccounts: micro.Wallet[] = [];
    ...
    private constructor(file: string) {
        ...
        const contextStr = process.env.MICRO_JEST_TEST_CONTEXT;
        if (!contextStr) {
        ...
        this.mainWallet = new micro.Wallet(suiteWalletPK, this.l2Provider, this.l1Provider);
    }
     ...
     */
    newEmptyAccount(): micro.Wallet {
        const randomPK = ethers.Wallet.createRandom().privateKey;
        const newWallet = new micro.Wallet(randomPK, this.l2Provider, this.l1Provider);
