...
testFees('Test fees', () => {
    ...
    beforeAll(() => {
        ...
        aliceErc20 = new ethers.Contract(tokenDetails.l1Address, micro.utils.IERC20, alice.ethWallet());
    });
...
async function killServerAndWaitForShutdown(provider: micro.Provider) {
...
async function setInternalL1GasPrice(provider: micro.Provider, newPrice?: string, disconnect?: boolean) {
    ...
    const microServer = utils.background(command, 'ignore');
