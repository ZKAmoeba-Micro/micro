...
import { EIP712_TX_TYPE, PRIORITY_OPERATION_L2_TX_TYPE } from 'micro-web3/build/src/utils';
...
describe('web3 API compatibility tests', () => {
    ...
    test('Should check the network version', async () => {
        // Valid network IDs for micro are greater than 270.
    ...
    test('Should test web3 response extensions', async () => {
        ...
        expect(tx1.chainId).toEqual(+process.env.CHAIN_ETH_MICRO_NETWORK_ID!);
        ...
        blockWithTransactions.transactions.forEach((txInBlock, _) => {
            ...
            expect(txInBlock.chainId).toEqual(+process.env.CHAIN_ETH_MICRO_NETWORK_ID!);
    ...
    test('Should test pub-sub API: txs', async () => {
        ...
        const tx = await alice.transfer({
            to: uniqueRecipient,
            amount: 1,
            token: micro.utils.ETH_ADDRESS // With ERC20 "to" would be an address of the contract.
        });
    ...
    test.skip('Should listen for human-readable events', async () => {
        ...
        for (let iter = 0; iter < 20; iter++) {
            ...
            await micro.utils.sleep(alice.provider.pollingInterval);
        }
    ...
    test('Should check metamask interoperability', async () => {
        ...
        const web3Provider = new micro.Web3Provider(from);
        ...
        const transfer = signer.transfer({
            ...
            overrides: {
                ...
                type: micro.utils.EIP712_TX_TYPE
            }
        });
    ...
    test('Should check sendRawTransaction returns GasPerPubDataLimitZero with 0 gas_per_pubdata_limit', async () => {
        ...
        const address = micro.Wallet.createRandom().address;
    ...
    test('Should check transaction signature', async () => {
        const CHAIN_ID = +process.env.CHAIN_ETH_MICRO_NETWORK_ID!;
        const value = 1;
...
export class MockMetamask {
    ...
    readonly networkVersion = parseInt(process.env.CHAIN_ETH_MICRO_NETWORK_ID!, 10);
    readonly chainId: string = ethers.utils.hexlify(parseInt(process.env.CHAIN_ETH_MICRO_NETWORK_ID!, 10));

    constructor(readonly wallet: micro.Wallet) {}
