...
import { BOOTLOADER_FORMAL_ADDRESS } from 'micro-web3/build/src/utils';
import fs from 'fs';

describe('Debug methods', () => {
    let testMaster: TestMaster;
    let alice: micro.Wallet;
    let bob: micro.Wallet;
    let tokenDetails: Token;
    let aliceErc20: micro.Contract;
    ...
    beforeAll(async () => {
        ...
        aliceErc20 = new micro.Contract(tokenDetails.l2Address, micro.utils.IERC20, alice);
    });

    test('Should not fail for infinity recursion', async () => {
        const bytecodePath = `${process.env.MICRO_HOME}/core/tests/ts-integration/contracts/zkasm/artifacts/deep_stak.zkasm/deep_stak.zkasm.zbin`;
        const bytecode = fs.readFileSync(bytecodePath);

        const contractFactory = new micro.ContractFactory([], bytecode, testMaster.mainAccount());
