...
describe('ERC20 contract checks', () => {
    ...
    test('Approve and transferFrom should work', async () => {
        ...
        const bobErc20 = new micro.Contract(tokenDetails.l2Address, micro.utils.IERC20, bob);

        // Fund bob's account to perform a transaction from it.
        await alice
            .transfer({ to: bob.address, amount: L2_ETH_PER_ACCOUNT.div(8), token: micro.utils.ETH_ADDRESS })
            .then((tx) => tx.wait());
    ...
    test('Should claim failed deposit', async () => {
        ...
        const l2Hash = micro.utils.getL2HashFromPriorityOp(l1Receipt, await alice.provider.getMainContractAddress());
