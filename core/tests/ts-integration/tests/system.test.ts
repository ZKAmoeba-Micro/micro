...
import { serialize, hashBytecode } from 'micro-web3/build/src/utils';
...
describe('System behavior checks', () => {
    ...
    test('Should check that system contracts and SDK create same CREATE/CREATE2 addresses', async () => {
        const deployerContract = new micro.Contract(
            micro.utils.CONTRACT_DEPLOYER_ADDRESS,
            micro.utils.CONTRACT_DEPLOYER,
            alice.provider
        );

        const sender = micro.Wallet.createRandom().address;
        const hash = ethers.utils.randomBytes(32);
        const salt = ethers.utils.randomBytes(32);
        const input = ethers.utils.randomBytes(128);
        const nonce = 5;

        const create2AddressBySDK = micro.utils.create2Address(sender, hash, salt, input);
        ...
        const createAddressBySDK = micro.utils.createAddress(sender, nonce);
    ...
    test('Should execute withdrawals with different parameters in one block', async () => {
        ...
        await alice
            .transfer({ amount: L2_ETH_PER_ACCOUNT.div(8), to: bob.address, token: micro.utils.ETH_ADDRESS })
            .then((tx) => tx.wait());
    ...
    function bootloaderUtilsContract() {
        ...
        const BOOTLOADER_UTILS = new ethers.utils.Interface(
            require(`${process.env.MICRO_HOME}/etc/system-contracts/artifacts-zk/cache-zk/solpp-generated-contracts/BootloaderUtilities.sol/BootloaderUtilities.json`).abi
        );
