...
const ETH_ADDRESS = micro.utils.ETH_ADDRESS;
...
describe('ETH token checks', () => {
    ...
    test('Can perform a deposit', async () => {
        ...
        const l2GasLimit = await micro.utils.estimateDefaultBridgeDepositL2Gas(
            alice.providerL1!,
            alice.provider,
            ETH_ADDRESS,
            amount,
        ...
        const gasPerPubdataByte = micro.utils.REQUIRED_L1_TO_L2_GAS_PER_PUBDATA_LIMIT;
    ...
    test('Can perform a transfer (EIP712)', async () => {
        ...
        const correctReceiptType = checkReceipt(
            (receipt) => receipt.type == micro.utils.EIP712_TX_TYPE,
            'Incorrect tx type in receipt'
        );
