...
import { getHashedL2ToL1Msg, L1_MESSENGER, L1_MESSENGER_ADDRESS } from '@zkamoeba/micro-web3/build/src/utils';
...
describe('Tests for L1 behavior', () => {
    ...
    let counterContract: micro.Contract;
    let contextContract: micro.Contract;
    let errorContract: micro.Contract;
    ...
    test('Should send L2->L1 messages', async () => {
        ...
        const contract = new micro.Contract(L1_MESSENGER_ADDRESS, L1_MESSENGER, alice);
        ...
        const l2ToL1LogIndex = receipt.l2ToL1Logs.findIndex(
            (log: micro.types.L2ToL1Log) => log.sender == L1_MESSENGER_ADDRESS
        );
        ...
        // Ensure that provided proof is accepted by the main micro contract.
        const microContract = await alice.getMainContract();
        const acceptedByContract = await microContract.proveL2MessageInclusion(
...
function maxL2GasLimitForPriorityTxs(): number {
    ...
    const overhead = getOverheadForTransaction(
        ...
        ethers.BigNumber.from(micro.utils.REQUIRED_L1_TO_L2_GAS_PER_PUBDATA_LIMIT),
