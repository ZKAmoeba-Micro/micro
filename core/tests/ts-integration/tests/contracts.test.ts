...
describe('Smart contract behavior checks', () => {
    ...
    test('Should not allow invalid contract bytecode', async () => {
        ...
        const send = (tx: any) => alice.provider.send('eth_sendRawTransaction', [micro.utils.serialize(tx)]);
        ...
        const longBytecodeLen = micro.utils.MAX_BYTECODE_LEN_BYTES + 32;
        const txWithTooLongBytecode = await invalidTx(longBytecodeLen);
        const tooLongBytecodeError = `Bytecode too long: ${longBytecodeLen} bytes, while max ${micro.utils.MAX_BYTECODE_LEN_BYTES} allowed`;
    ...
    test('Should interchangeably use ethers for eth calls', async () => {
        ...
        // to do an `eth_Call` and send transactions to micro contract.
