...
This folder contains a framework for writing integration tests for zkAmoeba, as well as set of integration test suites.
...
Basically, during initialization, everything is prepared for writing tests that interact with micro.
...
tests with the `MICRO_DEBUG_LOGS` environment variable set.
...
```typescript
// Create promise
const withdrawalPromise = alice.withdraw({ token: micro.utils.ETH_ADDRESS, amount });
