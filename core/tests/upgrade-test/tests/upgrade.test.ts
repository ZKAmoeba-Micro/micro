...
import fs from 'fs';
import { TransactionResponse } from '@zkamoeba/micro-web3/build/src/types';
...
const L2_FORCE_DEPLOY_UPGRADER_ABI = new ethers.utils.Interface(
    require(`${process.env.MICRO_HOME}/contracts/micro/artifacts-zk/cache-zk/solpp-generated-contracts/ForceDeployUpgrader.sol/ForceDeployUpgrader.json`).abi
);
const COMPLEX_UPGRADER_ABI = new ethers.utils.Interface(
    require(`${process.env.MICRO_HOME}/etc/system-contracts/artifacts-zk/cache-zk/solpp-generated-contracts/ComplexUpgrader.sol/ComplexUpgrader.json`).abi
);
const COUNTER_BYTECODE =
    require(`${process.env.MICRO_HOME}/core/tests/ts-integration/artifacts-zk/contracts/counter/counter.sol/Counter.json`).deployedBytecode;
...
describe('Upgrade test', function () {
    ...
    step('Run server and execute some transactions', async () => {
        ...
        try {
            await utils.exec('pkill micro_server');
        ...
        utils.background(
            'cd $MICRO_HOME && cargo run --bin micro_server --release -- --components=api,tree,eth,data_fetcher,state_keeper',
            [null, logs, logs]
        );
    ...
    step('Send l1 tx for saving new bootloader', async () => {
        const path = `${process.env.MICRO_HOME}/etc/system-contracts/bootloader/build/artifacts/playground_batch.yul/playground_batch.yul.zbin`;
...
async function prepareUpgradeCalldata(
    ...
    const micro = new ethers.Contract(microContract, zkweb3.utils.MICRO_MAIN_ABI, govWallet);

    const newProtocolVersion = params.newProtocolVersion ?? (await micro.getProtocolVersion()).add(1);
