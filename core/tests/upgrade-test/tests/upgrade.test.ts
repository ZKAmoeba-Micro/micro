...
import { IMicroFactory } from 'micro-web3/build/typechain';
...
describe('Upgrade test', function () {
    ...
    step('run server and execute some transactions', async () => {
        ...
        utils.background(
            `cd $MICRO_HOME && cargo run --bin micro_server --release -- --components=api,tree,eth,data_fetcher,state_keeper > server_logs.txt`
        );
    ...
    step('Send l1 tx for saving new bootloader', async () => {
        const path = `${process.env.MICRO_HOME}/etc/system-contracts/bootloader/build/artifacts/playground_block.yul/playground_block.yul.zbin`;
    ...
    step('upgrade bootloader on contract', async () => {
        ...
        const contract = IMicroFactory.connect(address, deployWallet);
