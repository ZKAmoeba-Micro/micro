...
pub fn deploy_erc20_tokens() -> Vec<Address> {
    ...
    let mut path = parse_env::<PathBuf>("MICRO_HOME");
    path.push("etc");
    path.push("tokens");
...
pub fn run_upgrade_contract(micro_address: Address, upgrade_gatekeeper_address: Address) {
    run_external_command(
        "zk",
        &[
            "run",
            "test-upgrade",
            &format!("0x{:x}", micro_address),
