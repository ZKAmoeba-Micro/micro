...
use micro_contracts::erc20_contract;
...
impl Tester {
    ...
    pub async fn change_config(&mut self, config: MicroConfig) {
    ...
    pub async fn deposit(
        ...
        self.account_balances.update_balance(
            to.address(),
            token,
            BalanceUpdate::new(LayerType::Micro, Sign::Plus, amount),
        );
    ...
    pub async fn transfer(
        ...
        self.account_balances.update_balance(
            from.address(),
            token,
            BalanceUpdate::new(LayerType::Micro, Sign::Minus, amount.clone()),
        );
        self.account_balances.update_balance(
            from.address(),
            ETHEREUM_ADDRESS,
    ...
    pub async fn assert_balances_correctness(&mut self) {
        ...
        for ((micro_account, token_address), expected_balance) in
            &self.account_balances.sync_accounts_balances
        {
            ...
            if &real_balance != expected_balance {
                println!(
                    "micro acc: {:x}, token address: {:x}",
                    micro_account, token_address
                );
                println!("expected:   {}", expected_balance);
                println!("real:       {}", real_balance);
                checks_failed = true;
