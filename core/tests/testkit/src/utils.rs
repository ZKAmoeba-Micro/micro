...
use micro_eth_client::{clients::http::SigningClient, EthInterface};
use micro_eth_signer::PrivateKeySigner;
use micro_types::{l1::L1Tx, web3::types::TransactionReceipt, Address};
...
pub fn load_test_bytecode_and_calldata() -> (Vec<u8>, Vec<u8>, Vec<u8>) {
    let mut dir_path = parse_env::<PathBuf>("MICRO_HOME");
