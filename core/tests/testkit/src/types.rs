...
use micro_types::web3::transports::Http;
use micro_types::L1ChainId;
use num::{bigint::Sign, BigUint, Zero};

use micro_config::MicroConfig;
// use micro_crypto::franklin_crypto::bellman::pairing::{
//     bn256::Fr,
//     ff::{PrimeField, PrimeFieldRepr},
// };
// use micro_prover_utils::precomputed::load_precomputed_proofs;
use micro_test_account::MicroAccount;
...
use micro_utils::u256_to_biguint;
...
impl AccountHandler {
    pub fn new(
        ...
        let sync_account = MicroAccount::new(private_key, Nonce(0));
    ...
    pub fn rand(config: &MicroConfig, micro_contract: Address) -> Self {
        let sync_account = MicroAccount::rand();
        let eth_provider = {
    ...
    pub fn update_balance(
        ...
        if account != self.operator_account.address() {
            let balance = match balance_update.layer_type {
                ...
                LayerType::Micro => self.sync_accounts_balances.get_mut(&(account, token)),
            }
            .unwrap();
