...
use micro_types::web3::{transports::Http, types::FilterBuilder};
use num::BigUint;
...
use micro_types::{l1::L1Tx, tokens::ETHEREUM_ADDRESS, Address, L1BatchNumber, H256};

pub fn get_test_config() -> MicroConfig {
...
/// Return Operator account and rich micro account with balance in L1.
...
async fn add_eth_token(tester: &mut Tester, account: &AccountHandler, contract_address: Address) {
    let new_priority_req_event_topic = micro_contract()
        .event("NewPriorityRequest")
        .expect("micro contract abi error")
        .signature();
...
pub async fn create_first_block(tester: &mut Tester, mut config: MicroConfig) {
...
pub async fn perform_transactions(
    ...
    micro_contract: Address,
    mut config: MicroConfig,
    ...
    let alice = AccountHandler::rand(&config, micro_contract);
    let bob = AccountHandler::rand(&config, micro_contract);
