...
use micro_eth_client::{types::Error, BoundEthInterface, EthInterface};
use micro_types::ethabi;
...
use micro_contracts::{erc20_contract, micro_contract};
use micro_eth_client::clients::http::SigningClient;
...
/// Used to sign and post ETH transactions for the micro contracts.
#[derive(Debug, Clone)]
...
impl EthereumProvider {
    ...
    pub async fn commit_blocks(
        ...
        let signed_tx = self
            ...
            .sign_prepared_tx(
                data,
                Options::with(|f| f.gas = Some(U256::from(9 * 10u64.pow(7)))),
                "provider",
            )
            .await
    ...
    pub async fn verify_blocks(
        ...
        let signed_tx = self
            ...
            .sign_prepared_tx(
                data,
                Options::with(|f| f.gas = Some(U256::from(10 * 10u64.pow(7)))),
                "provider",
            )
            .await
