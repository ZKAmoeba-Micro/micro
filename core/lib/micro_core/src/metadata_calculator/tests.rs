...
use micro_config::configs::{chain::OperationsManagerConfig, database::MerkleTreeConfig};
use micro_contracts::BaseSystemContracts;
use micro_dal::{ConnectionPool, StorageProcessor};
use micro_health_check::{CheckHealth, HealthStatus};
...
async fn expected_tree_hash(pool: &ConnectionPool) -> H256 {
    ...
    MicroTree::process_genesis_batch(&all_logs).root_hash
}
