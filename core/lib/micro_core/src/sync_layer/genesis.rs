...
use anyhow::Context;
use micro_contracts::{BaseSystemContracts, BaseSystemContractsHashes, SystemContractCode};
...
pub async fn perform_genesis_if_needed(
    ...
    let genesis_block_hash = if transaction.blocks_dal().is_genesis_needed().await.unwrap() {
        ...
        ensure_genesis_state(&mut transaction, micro_chain_id, &genesis_params)
            .await
...
async fn fetch_genesis_system_contracts(
    ...
    let client = HttpClientBuilder::default().build(main_node_url).unwrap();
    let hashes = client
        .sync_l2_block(micro_types::MiniblockNumber(0), false)
        .await?
...
pub async fn fetch_system_contract_by_hash(
    ...
    assert_eq!(
        hash,
        micro_utils::bytecode::hash_bytecode(&bytecode),
        "Got invalid base system contract bytecode from main node"
    );
    Ok(SystemContractCode {
        code: micro_utils::bytes_to_be_words(bytecode),
        hash,
    })
}
