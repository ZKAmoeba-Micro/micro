...
use micro_contracts::{governance_contract, BaseSystemContracts};
        ...
        if matches!(contracts_config.prover_at_genesis, ProverAtGenesis::Fri) {
            ...
            let vk_hash: micro_types::H256 = eth_client
                .call_contract_function(
                    ...
                    micro_contracts::verifier_contract(),
                )
                .await?;
...
pub async fn initialize_components(
    ...
    let main_micro_contract_address = contracts_config.diamond_proxy_addr;
...
async fn add_basic_witness_input_producer_to_task_futures(
    ...
    // Witness Generator won't be spawned with `MICRO_LOCAL_SETUP` running.
