...
use micro_types::zkevm_test_harness::blake2::Blake2s256;
use micro_types::zkevm_test_harness::witness::tree::BinaryHasher;
use micro_types::zkevm_test_harness::witness::tree::{
    BinarySparseStorageTree, EnumeratedBinaryLeaf, LeafQuery, MicroStorageLeaf,
};
...
impl BinarySparseStorageTree<256, 32, 32, 8, 32, Blake2s256, MicroStorageLeaf>
    for PrecalculatedMerklePathsProvider
{
    ...
    fn get_leaf(&mut self, index: &[u8; 32]) -> LeafQuery<256, 32, 32, 32, MicroStorageLeaf> {
        tracing::trace!(
    ...
    fn insert_leaf(
        ...
        index: &[u8; 32],
        leaf: MicroStorageLeaf,
    ) -> LeafQuery<256, 32, 32, 32, MicroStorageLeaf> {
        tracing::trace!(
    ...
    fn filter_renumerate<'a>(
        ...
        mut leafs: impl Iterator<Item = MicroStorageLeaf>,
    ) -> (
        u64,
        Vec<([u8; 32], MicroStorageLeaf)>,
        Vec<MicroStorageLeaf>,
    ) {
        tracing::trace!(
    ...
    fn verify_inclusion(
        root: &[u8; 32],
        query: &LeafQuery<256, 32, 32, 32, MicroStorageLeaf>,
    ) -> bool {
