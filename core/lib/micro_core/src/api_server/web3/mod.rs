...
use micro_health_check::{HealthStatus, HealthUpdater, ReactiveHealthCheck};
use micro_types::{api, MiniblockNumber};
use micro_web3_decl::{
    error::Web3Error,
    jsonrpsee::{
...
impl<G: 'static + Send + Sync + L1GasPriceProvider> ApiBuilder<G> {
    ...
    async fn build_rpc_module(mut self) -> RpcModule<()> {
        ...
        let micro_network_id = self.config.l2_chain_id;
        ...
        if namespaces.contains(&Namespace::Net) {
            rpc.merge(NetNamespace::new(micro_network_id).into_rpc())
        ...
        S: jsonrpc_core::Middleware<T>,
    {
        ...
        if namespaces.contains(&Namespace::Net) {
            io.extend_with(NetNamespace::new(micro_network_id).to_delegate());
        }
        if namespaces.contains(&Namespace::Debug) {
