...
use micro_concurrency::{ctx, scope, sync, time};
use micro_consensus_roles::validator;
use micro_contracts::{BaseSystemContractsHashes, SystemContractCode};
...
use crate::{
    ...
    state_keeper::{
        ...
        MicroStateKeeper, MiniblockSealer,
    },
    sync_layer::{
...
impl StateKeeperRunner {
    pub async fn run(self, ctx: &ctx::Ctx, pool: &ConnectionPool) -> anyhow::Result<()> {
        scope::run!(ctx, |ctx, s| async {
            ...
            s.spawn_bg(
                MicroStateKeeper::without_sealer(
                    stop_receiver,
                    Box::new(io),
