...
use micro_concurrency::{ctx, error::Wrap as _, sync, time};
use micro_consensus_bft::PayloadSource;
use micro_consensus_roles::validator;
use micro_consensus_storage::{BlockStore, ReplicaState, ReplicaStateStore, WriteBlockStore};
use micro_dal::{blocks_dal::ConsensusBlockFields, ConnectionPool};
use micro_types::{api::en::SyncBlock, Address, MiniblockNumber};
...
/// Context-aware `micro_dal::StorageProcessor` wrapper.
pub(super) struct StorageProcessor<'a>(micro_dal::StorageProcessor<'a>);
