...
use crate::eth_sender::{
    grafana_metrics::track_eth_tx_metrics, micro_functions::MicroFunctions, Aggregator,
    ETHSenderError,
};
...
pub struct EthTxAggregator {
    ...
    pub(super) main_micro_contract_address: Address,
...
impl EthTxAggregator {
    pub fn new(
        ...
        timelock_contract_address: Address,
        l1_multicall3_address: Address,
        main_micro_contract_address: Address,
        base_nonce: u64,
    ) -> Self {
        let functions = MicroFunctions::default();
        Self {
            config,
            aggregator,
    ...
    pub(super) fn generate_calldata_for_multicall(&self) -> Vec<Token> {
        ...
        let get_bootloader_hash_call = Multicall3Call {
            target: self.main_micro_contract_address,
            allow_failure: ALLOW_FAILURE,
            calldata: get_l2_bootloader_hash_input,
        };

        // Second micro contract call
