use micro_state::WriteStorage;
use micro_types::{l2_to_l1_log::L2ToL1Log, Timestamp, VmEvent};

use crate::{
    old_vm::events::merge_events, types::internals::MicroVmState, HistoryMode, L1BatchEnv,
};

pub(crate) fn collect_events_and_l1_logs_after_timestamp<S: WriteStorage, H: HistoryMode>(
    vm_state: &MicroVmState<S, H>,
    batch_env: &L1BatchEnv,
    from_timestamp: Timestamp,
) -> (Vec<VmEvent>, Vec<L2ToL1Log>) {
    let (raw_events, l1_messages) = vm_state
        .event_sink
        .get_events_and_l2_l1_logs_after_timestamp(from_timestamp);
    let events = merge_events(raw_events)
        .into_iter()
        .map(|e| e.into_vm_event(batch_env.number))
        .collect();
    (events, l1_messages.into_iter().map(Into::into).collect())
}
