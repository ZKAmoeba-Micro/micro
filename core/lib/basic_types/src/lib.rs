//! The declaration of the most primitive types used in micro network.
//!
...
basic_type!(
    /// micro network block sequential index.
    MiniblockNumber,
    u32
);

basic_type!(
    /// micro L1 batch sequential index.
    L1BatchNumber,
    u32
);
...
basic_type!(
    /// Unique identifier of the priority operation in the micro network.
    PriorityOpId,
    u64
);
