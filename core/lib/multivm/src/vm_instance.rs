...
use micro_state::{ReadStorage, StoragePtr, StorageView};
use micro_types::VmVersion;
...
impl<'a, S: ReadStorage, H: HistoryMode> VmInstance<'a, S, H> {
    ...
    pub fn push_transaction(&mut self, tx: &micro_types::Transaction) {
        match &mut self.vm {
...
pub struct VmVirtualBlocksNecessaryData<S: ReadStorage, H: HistoryMode> {
    pub storage_view: micro_state::StoragePtr<StorageView<S>>,
    pub history_mode: H,
}
