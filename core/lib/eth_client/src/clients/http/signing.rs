...
use micro_eth_signer::{raw_ethereum_tx::TransactionParameters, EthereumSigner};
...
use micro_types::{L1ChainId, PackedEthSignature, EIP_1559_TX_TYPE};
...
impl PKSigningClient {
    pub fn from_config(config: &MicroConfig) -> Self {
...
impl<S: EthereumSigner> BoundEthInterface for SigningClient<S> {
    ...
    async fn sign_prepared_tx_for_addr(
        ...
        let hash = micro_types::web3::signing::keccak256(&signed_tx).into();
