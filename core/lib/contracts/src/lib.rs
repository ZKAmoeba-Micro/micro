...
//!
//! Careful: some of the methods are reading the contracts based on the MICRO_HOME environment variable.
...
use micro_utils::{bytecode::hash_bytecode, bytes_to_be_words};
...
const MICRO_CONTRACT_FILE: &str =
    "contracts/ethereum/artifacts/cache/solpp-generated-contracts/micro/interfaces/IMicro.sol/IMicro.json";
...
const L2_BRIDGE_CONTRACT_FILE: &str =
    "contracts/micro/artifacts-zk/cache-zk/solpp-generated-contracts/bridge/interfaces/IL2Bridge.sol/IL2Bridge.json";
...
fn read_file_to_json_value(path: impl AsRef<Path>) -> serde_json::Value {
    ...
    let path = Path::new(&micro_home).join(path);
...
pub fn micro_contract() -> Contract {
    load_contract(MICRO_CONTRACT_FILE)
}
...
/// Reads bytecode from the path RELATIVE to the MICRO_HOME environment variable.
pub fn read_bytecode(relative_path: impl AsRef<Path>) -> Vec<u8> {
    let micro_home = std::env::var("MICRO_HOME").unwrap_or_else(|_| ".".into());
    let artifact_path = Path::new(&micro_home).join(relative_path);
...
pub fn default_erc20_bytecode() -> Vec<u8> {
    read_bytecode("etc/ERC20/artifacts-zk/contracts/MicroERC20.sol/MicroERC20.json")
}
...
/// As most of the static methods in this file, is loading data based on MICRO_HOME environment variable.
...
impl SystemContractsRepo {
    /// Returns the default system contracts repository with directory based on the MICRO_HOME environment variable.
    pub fn from_env() -> Self {
        let micro_home = std::env::var("MICRO_HOME").unwrap_or_else(|_| ".".into());
        let micro_home = PathBuf::from(micro_home);
        SystemContractsRepo {
            root: micro_home.join("etc/system-contracts"),
        }
    }
...
/// Reads zbin bytecode from a given path, relative to MICRO_HOME.
pub fn read_zbin_bytecode(relative_zbin_path: impl AsRef<Path>) -> Vec<u8> {
    let micro_home = std::env::var("MICRO_HOME").unwrap_or_else(|_| ".".into());
    let bytecode_path = Path::new(&micro_home).join(relative_zbin_path);
