...
/// If a peer has a micro score below this constant all other score parts will get ignored and
/// the peer will get banned regardless of the other parts.
const MIN_MICRO_SCORE_BEFORE_BAN: f64 = -60.0;
...
impl RealScore {
    ...
    fn recompute_score(&mut self) {
        self.score = self.micro_score;
        if self.micro_score <= MIN_MICRO_SCORE_BEFORE_BAN {
    ...
    pub fn apply_peer_action(&mut self, peer_action: PeerAction) {
        match peer_action {
            PeerAction::Fatal => self.set_micro_score(MIN_SCORE), // The worst possible score
    ...
    fn set_micro_score(&mut self, new_score: f64) {
        self.micro_score = new_score;
        self.update_state();
    }

    /// Add an f64 to the score abiding by the limits.
    fn add(&mut self, score: f64) {
        let new_score = (self.micro_score + score).clamp(MIN_SCORE, MAX_SCORE);
        self.set_micro_score(new_score);
    }
