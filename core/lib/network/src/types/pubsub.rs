...
use super::signed_block::SignedBlock;
            ...
            Ok(gossip_topic) => {
                ...
                match gossip_topic.kind() {
                    GossipKind::BeaconBlock => {
                        let beacon_block =
                            SignedBlock::from_ssz_bytes(data).map_err(|e| format!("{:?}", e))?;
...
impl std::fmt::Display for PubsubMessage {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            PubsubMessage::BeaconBlock(block) => write!(f, "Beacon Block: slot: {}", block.slot,),
        }
    }
}
