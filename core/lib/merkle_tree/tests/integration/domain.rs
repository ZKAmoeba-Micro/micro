...
use micro_merkle_tree::{domain::MicroTree, HashTree, TreeEntry, TreeInstruction};
use micro_storage::RocksDB;
use micro_system_constants::ACCOUNT_CODE_STORAGE_ADDRESS;
use micro_types::{
...
fn filtering_out_no_op_writes() {
    let temp_dir = TempDir::new().expect("failed get temporary directory for RocksDB");
    let db = RocksDB::new(temp_dir.as_ref());
    let mut tree = MicroTree::new(db.into());
...
fn revert_blocks() {
    ...
    let tree_metadata: Vec<_> = {
        let mut tree = MicroTree::new_lightweight(storage.into());
    ...
    let storage = RocksDB::new(temp_dir.as_ref());
    let tree = MicroTree::new_lightweight(storage.into());
...
fn process_block_idempotency_check() {
    ...
    let mut tree = MicroTree::new_lightweight(rocks_db.into());
    let logs = gen_storage_logs();
