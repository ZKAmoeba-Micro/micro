...
impl MicroTree {
    ...
    pub fn reader(&self) -> MicroTreeReader {
        let db = self.tree.db.inner().clone();
        MicroTreeReader(MerkleTree::new(db))
    }
...
/// Readonly handle to a [`MicroTree`].
#[derive(Debug)]
pub struct MicroTreeReader(MerkleTree<RocksDBWrapper>);

// While cloning `MerkleTree` is logically unsound, cloning a reader is reasonable since it is readonly.
impl Clone for MicroTreeReader {
    fn clone(&self) -> Self {
        Self(MerkleTree::new(self.0.db.clone()))
    }
}

impl MicroTreeReader {
    /// Returns the current root hash of this tree.
    pub fn root_hash(&self) -> ValueHash {
