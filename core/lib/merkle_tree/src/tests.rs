...
use crate::{MicroTree, TreeMetadata};
...
fn multiple_single_block_workflow() {
    ...
    let db = RocksDB::new(Database::MerkleTree, temp_dir.as_ref(), false);
    let mirror_tree = MicroTree::new(db);
...
fn revert_blocks() {
    ...
    let tree_metadata = {
        let mut tree = MicroTree::new(storage);
    ...
    let storage = RocksDB::new(Database::MerkleTree, temp_dir.as_ref(), false);
    let tree = MicroTree::new(storage);
...
fn process_block_idempotency_check() {
    ...
    let mut tree = MicroTree::new(rocks_db);
