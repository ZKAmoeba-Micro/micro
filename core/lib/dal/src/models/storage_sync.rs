...
use micro_protobuf::{read_required, ProtoFmt};
use micro_types::{api::en, Address, L1BatchNumber, MiniblockNumber, Transaction, H160, H256};
            ...
            consensus: match self.consensus {
                ...
                Some(v) => {
                    let v: ConsensusBlockFields =
                        micro_protobuf::serde::deserialize(v).context("consensus")?;
                    Some(v.encode())
                }
            },
        })
...
impl ConsensusBlockFields {
    pub fn decode(src: &en::ConsensusBlockFields) -> anyhow::Result<Self> {
        micro_protobuf::decode(&src.0 .0)
    }

    pub fn encode(&self) -> en::ConsensusBlockFields {
        en::ConsensusBlockFields(micro_protobuf::encode(self).into())
    }
}
