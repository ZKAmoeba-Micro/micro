...
use micro_config::constants::EMPTY_UNCLES_HASH;
...
use micro_types::api::{self, Block, BlockId, TransactionVariant};
use micro_types::l2_to_l1_log::L2ToL1Log;
use micro_types::vm_trace::Call;
use micro_types::web3::types::{BlockHeader, U64};
use micro_types::{L1BatchNumber, L2ChainId, MiniblockNumber, H160, H256, U256};
use micro_utils::{bigdecimal_to_u256, miniblock_hash};
...
impl BlocksWeb3Dal<'_, '_> {
    ...
    pub fn get_sealed_l2_miniblok_number(&mut self) -> Result<MiniblockNumber, SqlxError> {
        async_std::task::block_on(async {
            let started_at = Instant::now();
            let number: i64 = sqlx::query!(r#"SELECT MAX(number) as "number" FROM miniblocks"#)
                .fetch_one(self.storage.conn())
                ...
                .number
                .expect("DAL invocation before genesis");
            metrics::histogram!("dal.request", started_at.elapsed(), "method" => "get_sealed_l2_minibloks_number");
            Ok(MiniblockNumber(number as u32))
        })
    }

    pub fn get_sealed_prove_miniblock_number(&mut self) -> Result<MiniblockNumber, SqlxError> {
        async_std::task::block_on(async {
            let started_at = Instant::now();
            ...
            .number
            .expect("DAL invocation before genesis");
            metrics::histogram!("dal.request", started_at.elapsed(), "method" => "get_sealed_prove_miniblock_number");
            Ok(MiniblockNumber(number as u32))
        })
    }
