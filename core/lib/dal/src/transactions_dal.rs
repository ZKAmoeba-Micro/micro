...
use micro_types::{get_nonce_key, U256};
use micro_types::{
...
impl TransactionsDal<'_, '_> {
    pub fn insert_transaction_l1(&mut self, tx: L1Tx, l1_block_number: L1BlockNumber) {
        async_std::task::block_on(async {
            ...
            .await;
            if let Err(e) = res {
                vlog::error!("insert transaction failed {:?}", e);
            }
        })
    }
    ...
    pub fn get_tx_memory(&mut self) -> u32 {
        async_std::task::block_on(async {
            let counts: i64 = sqlx::query!(
                r#"SELECT COUNT(*) as "count!" FROM transactions WHERE in_mempool=TRUE"#
            )
