...
use micro_types::{L1BatchNumber, ProtocolVersionId};
...
impl WitnessGeneratorDal<'_, '_> {
    ...
    pub async fn get_next_leaf_aggregation_witness_job(
        ...
        if let Some(row) = record {
            ...
            let basic_circuits_proofs: Vec<
                Proof<Bn256, MicroCircuit<Bn256, VmWitnessOracle<Bn256>>>,
            > = self
    ...
    ) -> Vec<Proof<Bn256, MicroCircuit<Bn256, VmWitnessOracle<Bn256>>>> {
        {
            sqlx::query!(
                        "
                ...
                .into_iter()
                .map(|row| {
                    MicroProof::into_proof(bincode::deserialize::<MicroProof<Bn256>>(
                        &row.result
                            .expect("prove_job with `successful` status has no result"),
                    )
                        .expect("cannot deserialize proof"))
                })
                .collect::<Vec<Proof<Bn256, MicroCircuit<Bn256, VmWitnessOracle<Bn256>>>>>()
        }
    }
