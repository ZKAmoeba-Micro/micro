...
/// Loads the log format from the environment variable according to the existing micro configuration scheme.
...
/// Loads the Sentry URL from the environment variable according to the existing micro configuration scheme.
...
/// Prepared the Sentry environment ID from the environment variable according to the existing micro configuration
/// scheme.
...
pub fn environment_from_env() -> Option<String> {
    ...
    let l2_network = std::env::var("CHAIN_ETH_MICRO_NETWORK").ok()?;
