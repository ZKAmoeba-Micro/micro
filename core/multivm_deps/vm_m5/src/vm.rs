...
use micro_types::vm_trace::VmExecutionTrace;
...
pub type MicroVmState<'a, S> = VmState<
    'a,
    StorageOracle<S>,
    SimpleMemory,
    InMemoryEventSink,
...
impl<'a, S: Storage> VmInstance<'a, S> {
    ...
    pub(crate) fn collect_events_and_l1_logs_after_timestamp(
        ...
        (
            ...
            l1_messages
                .into_iter()
                .map(|log| L2ToL1Log::from(GlueInto::<micro_types::EventMessage>::glue_into(log)))
                .collect(),
        )
    }
        ...
        match stop_reason {
            VmExecutionStopReason::VmFinished => {
                ...
                full_result.l2_to_l1_logs = l1_messages
                    .into_iter()
                    .map(|log| {
                        L2ToL1Log::from(GlueInto::<micro_types::EventMessage>::glue_into(log))
                    })
                    .collect();
                VmBlockResult {
                    full_result,
...
// has failed.
pub(crate) fn tx_has_failed<S: Storage>(state: &MicroVmState<'_, S>, tx_id: u32) -> bool {
