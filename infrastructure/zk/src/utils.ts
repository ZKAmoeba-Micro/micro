...
export async function confirmAction() {
    if (process.env.MICRO_ACTION == 'dont_ask') return;
    ...
    const input = await new Promise((resolve) => {
        rl.question(
            'Dangerous action! (set MICRO_ACTION=dont_ask to always allow)\n' +
                `Type environment name (${process.env.MICRO_ENV}) to confirm: `,
            (input) => {
                rl.close();
                resolve(input);
            }
    ...
    if (input !== process.env.MICRO_ENV) {
...
export async function readMicroAbi() {
    const micro = process.env.MICRO_HOME;
    const path = `${micro}/contracts/artifacts/cache/solpp-generated-contracts/interfaces/IMicro.sol/IMicro.json`;
