...
enum BaseNetwork {
    LOCALHOST = 'localhost (zkamoeba/geth)',
...
async function initHyperchain() {
    ...
    // if we used zkamoeba/geth network, we need custom ENV file for hyperchain compose parts
    ...
    if (isLocalhost) {
        ...
        wrapEnvModify('DATABASE_URL', 'postgres://postgres:notsecurepassword@postgres:5432/micro_local');
    }
...
async function setHyperchainMetadata() {
    ...
    if (results.l1Chain !== BaseNetwork.LOCALHOST) {
        ...
        connectionsQuestions.push({
            ...
            name: 'dbUrl',
            type: 'input',
            initial: 'postgres://postgres@localhost/micro_local',
            required: true
        });
        ...
        l1Id = 9;
        databaseUrl = 'postgres://postgres:notsecurepassword@localhost:5432/micro_local';
    ...
    wrapEnvModify('CHAIN_ETH_MICRO_NETWORK', results.chainName);
    wrapEnvModify('CHAIN_ETH_MICRO_NETWORK_ID', results.chainId);
    ...
    if (results.deployWeth) {
        ...
        if (!process.env.DEPLOY_TEST_TOKENS) {
            ...
            if (!baseWethToken) {
                ...
                if (fs.existsSync(`/etc/tokens/${getEnv(process.env.MICRO_ENV!)}.json`)) {
                    tokens.push({
                        name: 'Wrapped Ether',
                        symbol: 'WETH',
                        decimals: 18,
                    ...
                    fs.writeFileSync(
                        `/etc/tokens/${getEnv(process.env.MICRO_ENV!)}.json`,
...
export function getTokens(network: string): L1Token[] {
    const configPath = `${process.env.MICRO_HOME}/etc/tokens/${network}.json`;
    try {
        return JSON.parse(
...
async function configDemoHyperchain(cmd: Command) {
    ...
    wrapEnvModify('CHAIN_ETH_MICRO_NETWORK', 'Zeek hyperchain');
    wrapEnvModify('CHAIN_ETH_MICRO_NETWORK_ID', '1337');
    wrapEnvModify('ETH_SENDER_SENDER_PROOF_SENDING_MODE', 'SkipEveryProof');
    wrapEnvModify('ETH_SENDER_SENDER_L1_BATCH_MIN_AGE_BEFORE_EXECUTE_SECONDS', '20');
