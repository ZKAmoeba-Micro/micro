...
export async function gitHooks() {
    if (fs.existsSync('.git')) {
        await utils.exec(`
            ...
            git config --local core.hooksPath ${process.env.MICRO_HOME}/.githooks
        `);
    }
}
...
export function load() {
    const microEnv = get();
    const envFile = (process.env.ENV_FILE = `etc/env/${microEnv}.env`);
    if (!fs.existsSync(envFile)) {
        // No .env file found - we should compile it!
...
export const command = new Command('env')
    .arguments('[env_name]')
    .description('get or set micro environment')
