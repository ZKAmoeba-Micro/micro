...
export async function all() {
    ...
    await utils.spawn('pkill micro_server');
...
export async function server(options: string[] = []) {
    if (process.env.MICRO_ENV?.startsWith('ext-node')) {
        ...
        process.env.MICRO_WEB3_WS_API_URL = `ws://127.0.0.1:${process.env.EN_WS_PORT}`;
        process.env.ETH_CLIENT_WEB3_URL = process.env.EN_ETH_CLIENT_URL;

        const configs = config.collectVariables(config.loadConfig(process.env.MICRO_ENV, true));
        ...
        process.env.CHAIN_ETH_MICRO_NETWORK_ID = configs.get('CHAIN_ETH_MICRO_NETWORK_ID');
    }
...
export async function testkit(args: string[], timeout: number) {
    ...
    await utils.spawn(`cargo run --release --bin micro_testkit -- ${args.join(' ')}`);
}

export async function rustSDK() {
    await utils.spawn('cargo test -p micro --release -- --ignored --test-threads=1');
}

export const command = new Command('integration').description('micro integration tests').alias('i');
