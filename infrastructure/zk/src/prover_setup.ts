    ...
    if (proverType == ProverType.GPU || proverType == ProverType.CPU) {
        ...
        if (!process.env.CI) {
            ...
            wrapEnvModify(
                ...
                `${process.env.MICRO_HOME}/etc/hyperchains/prover-keys/${process.env.MICRO_ENV}/${
...
async function downloadCSR(proverType: ProverType, region: string) {
    ...
    fs.mkdirSync(`${process.env.MICRO_HOME}/etc/hyperchains/prover-keys/${currentEnv}/${proverType}/`, {
        recursive: true
    });
    process.chdir(`${process.env.MICRO_HOME}/etc/hyperchains/prover-keys/${currentEnv}/${proverType}/`);
    console.log(chalk.yellow('Downloading ceremony (CSR) file'));
    await utils.spawn(
        `wget -q --show-progress -c https://storage.googleapis.com/zkamoeba-setup-keys-${region}/setup-keys/setup_2^24.key`
    );
    await utils.sleep(1);
    process.chdir(process.env.MICRO_HOME as string);
    wrapEnvModify('CRS_FILE', `${process.env.MICRO_HOME}/etc/hyperchains/prover-keys/${currentEnv}/${proverType}/`);
}
...
async function setupArtifactsMode() {
    if (process.env.CI) {
        const currentEnv = env.get();
        const path = `${process.env.MICRO_HOME}/etc/hyperchains/artifacts/${currentEnv}/`;
        ...
        const folderQuestion: BasePromptOptions[] = [
            {
                ...
                type: 'input',
                required: true,
                initial: `${process.env.MICRO_HOME}/etc/hyperchains/artifacts/${currentEnv}/`
            }
        ];
...
async function generateSetupData(isBaseLayer: boolean, proverType: ProverType) {
    ...
    const proverKeysDir = `${process.env.MICRO_HOME}/etc/hyperchains/prover-keys/${currentEnv}/${proverType}/`;
    fs.mkdirSync(proverKeysDir, { recursive: true });
    const proverDir = `${process.env.MICRO_HOME}/prover`;
    process.chdir(proverDir);
    const range = isBaseLayer ? 13 : 15;
    const gpuFeatureFlag = proverType == ProverType.GPU ? '--features "gpu"' : '';
    for (let i = 1; i <= range; i++) {
        const spawnCommand = `zk f cargo run ${gpuFeatureFlag} --release --bin micro_setup_data_generator_fri -- --numeric-circuit ${i} ${
...
async function downloadDefaultSetupKeys(proverType: ProverType, region: string) {
    const proverKeysUrls = require(`${process.env.MICRO_HOME}/prover/setup-data-${proverType}-keys.json`);
    const currentEnv = env.get();
    await downloadFilesFromGCP(
        proverKeysUrls[region],
        `${process.env.MICRO_HOME}/etc/hyperchains/prover-keys/${currentEnv}/${proverType}/`
    );

    await utils.spawn(
        `cp -r ${process.env.MICRO_HOME}/prover/vk_setup_data_generator_server_fri/data/* ${process.env.MICRO_HOME}/etc/hyperchains/prover-keys/${currentEnv}/${proverType}/`
    );
}
