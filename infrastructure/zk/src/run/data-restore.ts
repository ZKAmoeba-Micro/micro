...
export async function restart() {
    await utils.spawn('cargo run --bin micro_data_restore --release -- --genesis --finite');
}

export async function resume() {
    await utils.spawn('cargo run --bin micro_data_restore --release -- --continue');
}
...
export async function check(expectedHash: string) {
    await utils.spawn(
        `cargo run --bin micro_data_restore --release -- --genesis --finite --final_hash ${expectedHash}`
    );
}
