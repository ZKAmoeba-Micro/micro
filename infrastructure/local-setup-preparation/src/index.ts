...
async function depositWithRichAccounts() {
    ...
    if (!process.env.CONTRACTS_DIAMOND_PROXY_ADDR) {
        throw new Error('micro L1 Main contract address was not found');
    }
    ...
    const contract = new ethers.Contract(process.env.CONTRACTS_DIAMOND_PROXY_ADDR, utils.MICRO_MAIN_ABI, ethProvider);
    ...
    for (const wallet of wallets) {
        const contract = new ethers.Contract(process.env.CONTRACTS_DIAMOND_PROXY_ADDR, utils.MICRO_MAIN_ABI, wallet);
