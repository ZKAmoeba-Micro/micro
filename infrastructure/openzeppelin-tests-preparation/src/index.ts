...
async function depositTestAccounts() {
    ...
    const web3Provider = new zkweb3.Provider(process.env.MICRO_WEB3_API_URL || 'http://localhost:3050');
...
async function main() {
    ...
    await sendBytecodeFromFolder(`${process.env.MICRO_HOME}/etc/openzeppelin-contracts/artifacts-zk`);
}
