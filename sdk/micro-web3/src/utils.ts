...
export const MICRO_MAIN_ABI = new utils.Interface(require('../../abi/IMicro.json').abi);
...
export function create2Address(sender: Address, bytecodeHash: BytesLike, salt: BytesLike, input: BytesLike) {
    const prefix = ethers.utils.keccak256(ethers.utils.toUtf8Bytes('microCreate2'));
...
export function createAddress(sender: Address, senderNonce: BigNumberish) {
    const prefix = ethers.utils.keccak256(ethers.utils.toUtf8Bytes('microCreate'));
    const addressBytes = ethers.utils
        .keccak256(
            ethers.utils.concat([
                prefix,
...
export function getL2HashFromPriorityOp(txReceipt: ethers.providers.TransactionReceipt, microAddress: Address): string {
    ...
    for (const log of txReceipt.logs) {
        ...
        try {
            const priorityQueueLog = MICRO_MAIN_ABI.parseLog(log);
