// //! Integration test for micro Rust SDK.
// //!
// //! In order to pass these tests, there must be a running
// //! instance of micro server and prover:
// //!
// //! ```bash
// //! zk server &!
// //! zk dummy-prover run &!
...
// const LOCALHOST_WEB3_ADDR: &str = "http://127.0.0.1:1234/rpc/v1";
...
// async fn test_tx_fail<S, P>(micro_depositor_wallet: &Wallet<S, P>) -> Result<(), anyhow::Error>
// where
//     S: EthereumSigner,
//     P: Provider + Clone,
// {
...
//         .start_transfer()
//         .to(micro_depositor_wallet.address())
//         .token("ETH")?
//         .amount(1_000_000u64)
//         .send()
//         .await;
...
// /// Makes a transfer from Alice to Bob inside micro
...
//     // Currently fast withdraw aren't supported by micro-rs, but they will be in the near future.
//     // test_fast_withdraw(eth, main_contract, &bob, &bob, &token, withdraw_amount);
//     // println!("Fast withdraw ok, Token: {}", token.symbol);

//     // Currently multi transactions aren't supported by micro-rs, but they will be in the near future.
...
//         signed_transfers.push((MicroTx::Transfer(Box::new(transfer)), signature));
