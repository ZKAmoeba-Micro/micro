...
use micro_types::{l1::L1Tx, Address, L1TxCommonData};
...
const MICRO_INTERFACE: &str = include_str!("../abi/IMicro.json");
...
/// Returns `ethabi::Contract` object for micro smart contract.
pub fn micro_contract() -> ethabi::Contract {
    load_contract(MICRO_INTERFACE)
}
...
impl<S: EthereumSigner> EthereumProvider<S> {
    ...
    /// Returns the micro contract address.
    ...
    /// Note: This operation is performed on Ethereum, and not related to micro directly.
    ...
    pub async fn base_cost(
        ...
        let gas_price = if let Some(gas_price) = gas_price {
            ...
            self.eth_client.get_gas_price("micro-rs").await?
        };
        self.eth_client
    ...
    /// Performs a deposit in micro network.
