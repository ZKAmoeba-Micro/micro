...
async function main() {
    ...
    program.version('0.1.0').name('deploy-getters').description('Deploys the getters to L2');
    ...
    program.option('--private-key <private-key>').action(async (cmd: Command) => {
        ...
        const gettersBytecode = hre.artifacts.readArtifactSync('Getters').bytecode;
        const create2Salt = ethers.constants.HashZero;
        const gettersAddress = computeL2Create2Address(deployWallet, gettersBytecode, '0x', create2Salt);

        // TODO: request from API how many L2 gas needs for the transaction.
        await create2DeployFromL1(deployWallet, gettersBytecode, '0x', create2Salt, priorityTxMaxGasLimit);

        console.log(`CONTRACTS_L2_GETTERS_ADDR=${gettersAddress}`);
    });
