...
import { IMicroFactory } from '../../ethereum/typechain/IMicroFactory';
...
const CREATE2_PREFIX = ethers.utils.solidityKeccak256(['string'], ['microCreate2']);
...
export async function create2DeployFromL1(
    ...
    const microAddress = deployedAddressesFromEnv().Micro.DiamondProxy;
    const micro = IMicroFactory.connect(microAddress, wallet);
    ...
    const gasPrice = await micro.provider.getGasPrice();
    const expectedCost = await micro.l2TransactionBaseCost(gasPrice, l2GasLimit, REQUIRED_L2_GAS_PRICE_PER_PUBDATA);

    await micro.requestL2Transaction(
