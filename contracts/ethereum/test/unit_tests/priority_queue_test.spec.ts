...
describe('Priority queue tests', function () {
    ...
    describe('on empty queue', function () {
        ...
        it('failed to get by index', async () => {
            const revertReason = await getCallRevertReason(priorityQueueTest.getByIndex(1));
            expect(revertReason).equal('O');
        });
    ...
    describe('push operations', function () {
        ...
        it('getByIndex', async () => {
            const frontElement = await priorityQueueTest.getByIndex(1);

            expect(frontElement.canonicalTxHash).equal(queue[1].canonicalTxHash);
            expect(frontElement.expirationTimestamp).equal(queue[1].expirationTimestamp);
            expect(frontElement.layer2Tip).equal(queue[1].layer2Tip);
        });
