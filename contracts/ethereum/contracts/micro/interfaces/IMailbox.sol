...
interface IMailbox {
    ...
    /// @notice New priority request event. Emitted when a request is placed into the priority queue
    /// @param txId Serial number of the priority operation
    /// @param depIndex Index of the factory deps
    /// @param splittedIndex Index of splitted factory dep
    /// @param factoryDep A bytecodes that were shown in the L1 public data. Will be marked as known bytecodes in L2
    event NewPriorityRequestFactoryDeps(uint256 txId, uint256 depIndex, uint256 splittedIndex, bytes factoryDep);
