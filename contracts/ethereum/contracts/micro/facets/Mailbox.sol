...
/// @title micro Mailbox contract providing interfaces for L1 <-> L2 interaction.
/// @author Matter Labs
contract MailboxFacet is Base, IMailbox {
    ...
    function requestL2Transaction(
        ...
        // Please note, currently micro address derivation is different from Ethereum one, but it may be changed in the future.
    ...
    /// @notice Request block event execution of L2 transaction from L1.
    /// @param _refundRecipient The address on L2 that will receive the refund for the transaction. If the transaction fails,
    /// @param _l2Calldata The input of the L2 transaction
    /// @param _factoryDeps An array of L2 bytecodes that will be marked as known on L2
    /// @return canonicalTxHash The hash of the requested L2 transaction. This hash can be used to follow the transaction status
    function onBlockEvent(
        address _refundRecipient,
        bytes calldata _l2Calldata,
        bytes[] calldata _factoryDeps
    ) external payable nonReentrant onlyValidator returns (bytes32 canonicalTxHash) {
        canonicalTxHash = _requestL2Transaction(
            L2_PROOF_REWARD_POOL_CONTRACT_CALLER_ADDR,
            L2_PROOF_REWARD_POOL_CONTRACT_ADDR,
            0,
            _l2Calldata,
            DEPLOY_L2_CONTRACT_TX_GAS_LIMIT,
    ...
    function _writePriorityOp(
        ...
        for (uint256 i = 0; i < factoryDepsLen; i = i.uncheckedInc()) {
            uint256 len = _factoryDeps[i].length;
            uint256 start = 0;
            uint256 j = 0;
            while (true) {
                uint256 end = start + 8000;
                if (end > len) {
                    emit NewPriorityRequestFactoryDeps(_priorityOpParams.txId, i, j, _factoryDeps[i][start:]);
                    break;
                } else {
                    emit NewPriorityRequestFactoryDeps(_priorityOpParams.txId, i, j, _factoryDeps[i][start:end]);
                }
                start = end;
                j = j.uncheckedInc();
            }
    ...
    /// of the fee model of micro. The appropriate comments are also present
