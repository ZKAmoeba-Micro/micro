...
const mainnetTokens = require(`${process.env.MICRO_HOME}/etc/tokens/mainnet`);
...
async function main() {
    ...
    if (args.publish) {
        ...
        // const networkTokens = require(`${process.env.MICRO_HOME}/etc/tokens/${process.env.ETH_NETWORK}`);
    ...
    fs.writeFileSync(
        `${process.env.MICRO_HOME}/etc/tokens/${process.env.CHAIN_ETH_NETWORK}.json`,
