...
const contractArtifactsPath = path.join(process.env.MICRO_HOME as string, 'contracts/micro/artifacts-zk/');
...
async function main() {
    ...
    program
        ...
        .action(async (cmd) => {
            ...
            console.log(`micro address: ${micro.address}, erc20bridge: ${erc20Bridge.address}`);

            const governorAddress = await micro.getGovernor();
            ...
            const requiredValueToInitializeBridge = await micro.l2TransactionBaseCost(
                gasPrice,
            ...
            const independentInitialization = [
                micro.requestL2Transaction(
                    ...
                    { gasPrice, nonce, value: requiredValueToPublishBytecodes, gasLimit: 1_000_000_000 }
                ),
