...
const microInterface = new Interface(hardhat.artifacts.readArtifactSync('IMicro').abi);
const verifierInterface = new Interface(hardhat.artifacts.readArtifactSync('Verifier').abi);

const interfaces = [erc20BridgeInterface, microInterface, verifierInterface];
...
async function reason() {
    ...
    if (!tx) {
        ...
        try {
            ...
            console.log('micro transaction is not parsed');
        }
