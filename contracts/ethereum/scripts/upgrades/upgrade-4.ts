...
import { Provider } from 'micro-web3';
...
async function prepareCalldata(
    ...
    const oldDeployerSystemContract = await ethers.getContractAt(
        'cache/solpp-generated-contracts/micro/upgrade-initializers/DiamondUpgradeInit4.sol:IOldContractDeployer',
        ZERO_ADDRESS
    );
...
async function main() {
    ...
    program
        ...
        .action(async (cmd) => {
            ...
            const microContract = deployer.microContract(deployWallet);
            ...
            const proposalId = cmd.proposalId ? cmd.proposalId : await microContract.getCurrentProposalId();
            const proposeUpgradeTx = await microContract.proposeTransparentUpgrade(upgradeParam, proposalId);
            await proposeUpgradeTx.wait();

            const executeUpgradeTx = await microContract.executeUpgrade(upgradeParam, ethers.constants.HashZero);
