...
async function main() {
    ...
    program
        ...
        .action(async (cmd) => {
            ...
            const microContract = IOldDiamondCutFactory.connect(deployer.addresses.Micro.DiamondProxy, deployWallet);
            ...
            const proposeUpgradeTx = await microContract.proposeDiamondCut(
                upgradeParam.facetCuts,
                upgradeParam.initAddress
            );
            await proposeUpgradeTx.wait();

            const executeUpgradeTx = await microContract.executeDiamondCutProposal(upgradeParam);
