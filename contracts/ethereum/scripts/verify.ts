...
async function main() {
    ...
    for (const address of [
        addresses.Micro.DiamondCutFacet,
        addresses.Micro.GettersFacet,
        addresses.Micro.DiamondInit,
        addresses.Micro.GovernanceFacet,
        addresses.Micro.MailboxFacet,
        addresses.Micro.ExecutorFacet,
        addresses.Micro.Verifier
    ]) {
    ...
    //     const promise = verifyPromise(addresses.Micro.DiamondProxy, constructorArguments);
    //     promises.push(promise);
    // }

    // Bridges
    const promise = verifyPromise(addresses.Bridges.ERC20BridgeImplementation, [
        addresses.Micro.DiamondProxy,
        addresses.AllowList
    ]);
    promises.push(promise);
