...
function getMicroContract() {
    ...
    return deployer.microContract(dummyWallet);
}
...
async function main() {
    ...
    program.command('diamond-loupe-view').action(async () => {
        const facets = await getMicroContract().facets();
    ...
    program
        ...
        .action(async (facetCutsData: string, cmd) => {
            ...
            const proposalId = cmd.proposalId
                ? cmd.proposalId
                : (await getMicroContract().getCurrentProposalId()).add(1);
