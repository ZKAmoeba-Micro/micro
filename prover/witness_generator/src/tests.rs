...
use zkevm_test_harness::witness::tree::{BinarySparseStorageTree, MicroStorageLeaf};
...
const LEAFS: [MicroStorageLeaf; 2] = [
    generate_leaf(
        1,
...
const fn generate_leaf(index: u64, value: &[u8]) -> MicroStorageLeaf {
    MicroStorageLeaf {
        index,
...
fn test_filter_renumerate_panic_when_leafs_and_indices_are_of_different_length() {
    const ANOTHER_LEAF: MicroStorageLeaf = generate_leaf(
        2,
