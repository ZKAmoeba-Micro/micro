...
if [[ -z "${MICRO_HOME}" ]]; then
  echo "Environment variable MICRO_HOME is not set. Make sure it's set and pointing to the root of this repository"
  exit 1
fi
...
do
    if ! [ -f vk_setup_data_generator_server_fri/data/setup_basic_${i}_data.bin ]; then
        zk f cargo run $GPU_FLAG --release --bin micro_setup_data_generator_fri -- --numeric-circuit $i --is_base_layer
    fi
done

if ! [ -f vk_setup_data_generator_server_fri/data/setup_scheduler_data.bin ]; then
    zk f cargo run $GPU_FLAG --release --bin micro_setup_data_generator_fri -- --numeric-circuit 1
fi

if ! [ -f vk_setup_data_generator_server_fri/data/setup_node_data.bin ]; then
    zk f cargo run $GPU_FLAG --release --bin micro_setup_data_generator_fri -- --numeric-circuit 2
fi

for i in {3..15}
do
    if ! [ -f vk_setup_data_generator_server_fri/data/setup_leaf_${i}_data.bin ]; then
        zk f cargo run $GPU_FLAG --release --bin micro_setup_data_generator_fri -- --numeric-circuit $i
    fi
done
