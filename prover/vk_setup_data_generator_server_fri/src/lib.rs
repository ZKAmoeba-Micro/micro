...
use circuit_definitions::circuit_definitions::aux_layer::{
    MicroCompressionLayerStorageType, MicroSnarkWrapperVK,
};
...
use micro_prover_fri_types::{
    circuit_definitions::{
        ...
        circuit_definitions::{
            base_layer::{MicroBaseLayerCircuit, MicroBaseLayerVerificationKey},
            recursion_layer::{MicroRecursionLayerStorageType, MicroRecursionLayerVerificationKey},
        },
...
pub fn get_base_layer_vk_for_circuit_type(
    circuit_type: u8,
) -> anyhow::Result<MicroBaseLayerVerificationKey> {
...
pub fn get_snark_vk(
) -> anyhow::Result<SnarkVerificationKey<Bn256, MicroCircuit<Bn256, SnarkWitnessOracle<Bn256>>>> {
    ...
    let text = fs::read_to_string(&filepath)
        .with_context(|| format!("Failed reading verification key from path: {filepath}"))?;
    serde_json::from_str::<
        SnarkVerificationKey<Bn256, MicroCircuit<Bn256, SnarkWitnessOracle<Bn256>>>,
    >(&text)
    .with_context(|| format!("Failed deserializing verification key from path: {filepath}"))
}
