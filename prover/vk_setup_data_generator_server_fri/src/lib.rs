...
use circuit_definitions::circuit_definitions::aux_layer::{MicroCompressionLayerStorageType, MicroSnarkWrapperVK};
use std::fs;
use std::fs::File;
use std::io::Read;
...
use micro_prover_fri_types::circuit_definitions::boojum::algebraic_props::sponge::GenericAlgebraicSponge;

use micro_prover_fri_types::circuit_definitions::boojum::cs::implementations::hints::{
    DenseVariablesCopyHint, DenseWitnessCopyHint,
};
use micro_prover_fri_types::circuit_definitions::boojum::cs::implementations::polynomial_storage::{
    SetupBaseStorage, SetupStorage,
};
use micro_prover_fri_types::circuit_definitions::boojum::cs::implementations::setup::FinalizationHintsForProver;
use micro_prover_fri_types::circuit_definitions::boojum::cs::implementations::verifier::VerificationKey;
use micro_prover_fri_types::circuit_definitions::boojum::cs::oracle::merkle_tree::MerkleTreeWithCap;
use micro_prover_fri_types::circuit_definitions::boojum::cs::oracle::TreeHasher;
use micro_prover_fri_types::circuit_definitions::boojum::field::goldilocks::GoldilocksField;
use micro_prover_fri_types::circuit_definitions::boojum::field::{PrimeField, SmallField};

use micro_prover_fri_types::circuit_definitions::boojum::field::traits::field_like::PrimeFieldLikeVectorized;
use micro_prover_fri_types::circuit_definitions::boojum::implementations::poseidon2::Poseidon2Goldilocks;
use micro_prover_fri_types::circuit_definitions::boojum::worker::Worker;

use micro_prover_fri_types::circuit_definitions::circuit_definitions::base_layer::{
    MicroBaseLayerCircuit, MicroBaseLayerVerificationKey,
};
use micro_prover_fri_types::circuit_definitions::circuit_definitions::recursion_layer::{
    MicroRecursionLayerStorageType, MicroRecursionLayerVerificationKey,
};
use micro_prover_fri_types::circuit_definitions::{
    MicroDefaultRoundFunction, BASE_LAYER_CAP_SIZE, BASE_LAYER_FRI_LDE_FACTOR,
};
...
use micro_types::zkevm_test_harness::bellman::plonk::better_better_cs::setup::VerificationKey as SnarkVerificationKey;
use micro_types::zkevm_test_harness::witness::oracle::VmWitnessOracle as SnarkWitnessOracle;
...
pub fn get_base_layer_vk_for_circuit_type(circuit_type: u8) -> anyhow::Result<MicroBaseLayerVerificationKey> {
    ...
    serde_json::from_str::<MicroBaseLayerVerificationKey>(&text)
...
pub fn get_recursive_layer_vk_for_circuit_type(
    circuit_type: u8,
) -> anyhow::Result<MicroRecursionLayerVerificationKey> {
    ...
    let text = std::fs::read_to_string(&filepath)
        .with_context(|| format!("Failed reading verification key from path: {filepath}"))?;
    serde_json::from_str::<MicroRecursionLayerVerificationKey>(&text)
        .with_context(|| format!("Failed deserializing verification key from path: {filepath}"))
}

pub fn get_round_for_recursive_circuit_type(circuit_type: u8) -> AggregationRound {
    match circuit_type {
        circuit_type if circuit_type == MicroRecursionLayerStorageType::SchedulerCircuit as u8 => {
            AggregationRound::Scheduler
        }
        circuit_type if circuit_type == MicroRecursionLayerStorageType::NodeLayerCircuit as u8 => {
...
pub fn save_base_layer_vk(vk: MicroBaseLayerVerificationKey) -> anyhow::Result<()> {
...
pub fn save_recursive_layer_vk(vk: MicroRecursionLayerVerificationKey) -> anyhow::Result<()> {
    let circuit_type = vk.numeric_circuit_type();
    let round = get_round_for_recursive_circuit_type(circuit_type);
    let filepath = get_file_path(
        ProverServiceDataKey::new(circuit_type, round),
...
pub fn save_snark_vk(vk: MicroSnarkWrapperVK) -> anyhow::Result<()> {
    let filepath = get_file_path(
...
pub fn get_finalization_hints(key: ProverServiceDataKey) -> anyhow::Result<FinalizationHintsForProver> {
    ...
    if key.round == AggregationRound::NodeAggregation {
        key.circuit_id = MicroRecursionLayerStorageType::NodeLayerCircuit as u8;
    }
...
pub fn get_snark_vk() -> anyhow::Result<SnarkVerificationKey<Bn256, MicroCircuit<Bn256, SnarkWitnessOracle<Bn256>>>> {
    let circuit_id = MicroCompressionLayerStorageType::CompressionMode1Circuit as u8;
    let filepath = get_file_path(
    ...
    let text = fs::read_to_string(&filepath)
        .with_context(|| format!("Failed reading verification key from path: {filepath}"))?;
    serde_json::from_str::<SnarkVerificationKey<Bn256, MicroCircuit<Bn256, SnarkWitnessOracle<Bn256>>>>(&text)
        .with_context(|| format!("Failed deserializing verification key from path: {filepath}"))
}
