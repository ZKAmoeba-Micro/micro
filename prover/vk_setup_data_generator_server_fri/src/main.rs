...
use circuit_definitions::circuit_definitions::recursion_layer::{
    MicroRecursionLayerProof, MicroRecursionLayerVerificationKey,
};
...
use micro_vk_setup_data_server_fri::utils::get_scheduler_proof_for_snark_vk_generation;
...
fn save_vks(source: &dyn SetupDataSource) -> anyhow::Result<()> {
    ...
    for leaf_circuit_type in (MicroRecursionLayerStorageType::LeafLayerCircuitForMainVM as u8)
    ...
    save_recursive_layer_vk(
        source
            .get_recursion_layer_vk(MicroRecursionLayerStorageType::SchedulerCircuit as u8)
...
fn generate_snark_vk(
    ...
    scheduler_vk: MicroRecursionLayerVerificationKey,
    compression_mode: u8,
) -> anyhow::Result<()> {
