...
use micro_prover_fri_types::circuit_definitions::circuit_definitions::base_layer::{
    MicroBaseLayerCircuit, MicroBaseLayerProof,
};
use micro_prover_fri_types::circuit_definitions::circuit_definitions::recursion_layer::{
    MicroRecursionLayerProof, MicroRecursiveLayerCircuit,
};
use micro_prover_fri_types::circuit_definitions::{
    base_layer_proof_config, recursion_layer_proof_config, MicroDefaultRoundFunction,
};
...
use micro_prover_fri_types::{CircuitWrapper, FriProofWrapper, ProverJob, ProverServiceDataKey};
use micro_prover_fri_utils::fetch_next_circuit;
...
impl Prover {
    ...
    fn prove_recursive_layer(
        job_id: u32,
        circuit: MicroRecursiveLayerCircuit,
        ...
        FriProofWrapper::Recursive(MicroRecursionLayerProof::from_inner(circuit_id, proof))
    }
    ...
    fn prove_base_layer(
        ...
        FriProofWrapper::Base(MicroBaseLayerProof::from_inner(circuit_id, proof))
    }
}
...
pub fn load_setup_data_cache(config: &FriProverConfig) -> anyhow::Result<SetupLoadMode> {
    Ok(match config.setup_load_mode {
        micro_config::configs::fri_prover::SetupLoadMode::FromDisk => SetupLoadMode::FromDisk,
        micro_config::configs::fri_prover::SetupLoadMode::FromMemory => {
